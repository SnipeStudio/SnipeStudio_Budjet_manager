  ...
  if(db->dbIsOpen())
    {
      ...
      dbOpenError->warning(this, tr("Error in Db Loading"), tr("There are some shit happens during database loading"));
      dbOpenError->exec();
    }
...
void Widget::load()
{
    ...
    translator.load(QDir::toNativeSeparators("translations/ssbm_" + data->getTranslation()+".qm"));
